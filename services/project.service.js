...
export const deleteProjectById = async (id) => {
	...
	try{
		...
		if (!team) {
			throw new Error(`Team with id ${project.team} not found`);
		}

		console.log('Removed project ${projectId} from team ${project.team}')
...
export const createProject = async (projectData) => {
	...
	try{
		...
		await TeamModel.findByIdAndUpdate(
			projectData.team,
			{$push: {team:newProject._id}},
			{new:true}
		)

		console.log('Project with id ${newProject._id} created successfully')
		return newProject
	} catch (error) {
